...
def execute_and_download_code(code: str, required_files: str = "") -> str:
    ...
    try:
        ...
        with Sandbox.create() as sandbox:
            ...
            # 1. Parse output files from ### OUTPUT_FILES marker
...
def process_single_result(result, output_dir: str, index: int, output_files: List[str]) -> List[str]:
    """
    Process a single E2B result and save any files it contains.
    If binary results exist, filenames are taken from output_files if available.
    """
    created_files = []
    
    def pick_filename(default_name: str) -> str:
        """Pick filename from output_files if available, else fallback."""
        if output_files and len(output_files) > index:
    ...
    try:
        ...
        if hasattr(result, '__class__') and 'PIL.' in str(result.__class__):
            ...
            try:
                ...
                filename = pick_filename(f"result_{index}.png")
        ...
        # Handle base64 JPG
            ...
            try:
                filename = pick_filename(f"result_{index}.jpg")
                filepath = os.path.join(output_dir, filename)
        ...
        # Handle SVG
            ...
            try:
                filename = pick_filename(f"result_{index}.svg")
                filepath = os.path.join(output_dir, filename)
        ...
        # Handle text results
            ...
            try:
                filename = pick_filename(f"result_{index}.txt")
                filepath = os.path.join(output_dir, filename)
